...
export const api = {
  ...
  bulkDeleteMatches: (ids: string[], signal?: AbortSignal) => post<{ results: { match_id: string; ok: boolean; error: string | null }[]; deleted: number; failed: number }>("/matches/bulk-delete", { match_ids: ids }, signal ? { signal } : undefined),
